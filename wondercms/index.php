...
EOT;
	...
	public function parseUrl(): string
	{
		...
		// Fallback for hosts where the .htaccess rewrite doesn't forward
		// ?page=... (e.g. Nginx/LiteSpeed catch-all rules, or mod_rewrite
		// disabled). Without this, every request that isn't the bare
		// domain falls through to the default (home) page below, even
		// though the URL in the browser looks correct.
		if (!$page) {
			$page = $this->parsePageFromRequestUri();
		}
	...
	/**
	 * Derive the requested page slug straight from REQUEST_URI when the
	 * webserver reached index.php without a ?page=... query string.
	 *
	 * @return string|null
	 */
	private function parsePageFromRequestUri(): ?string
	{
		...
		$path = trim($path, '/');
		if ($path === '' || strtolower(basename($path)) === 'index.php') {
			return null;
		}

		return trim(htmlspecialchars($path, ENT_QUOTES));
	}
